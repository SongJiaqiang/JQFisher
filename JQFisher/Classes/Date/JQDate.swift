...
public extension Date {
    ...
    static func differenceInDaysWithDate(startDate: Date, endDate: Date) -> Int? {
        let calendar = Calendar(identifier: .gregorian)
        let fromDate = calendar.startOfDay(for: startDate)
        let toDate = calendar.startOfDay(for: endDate)
        let components = calendar.dateComponents([.day], from: fromDate, to: toDate)
        if let days = components.day {
//            print("两日期的绝对天数差：\(days)")
            return days
        }
        return nil
    }
...
public extension Date {
    ...
    func dayBefore(interval: Int) -> Date {
        return Calendar.current.date(byAdding: .day, value: -interval, to: noon)!
    }
    
    func dayAfter(interval: Int) -> Date {
        return Calendar.current.date(byAdding: .day, value: interval, to: noon)!
    }
}


/// Copy from APRichAlert
public extension Date {
